...
4. Copy your secret seed phrase and put it in a secure location, then click **Next**.
5. Type an account name and password, then click **Next**.

6. Click **Save**.

7. Join the [Rococo Element channel](https://matrix.to/#/#rococo-faucet:matrix.org) and send a message with`!drip` and the public address for your Rococo to get 100 ROC in your wallet.
...
1. Click **Parathreads**, then click **ParaId**.

1. Review the transaction and note the parachain identifier that you are assigned, then click **Submit**.
1. Type your password to authenticate your identity, then click **Sign and Submit**.

   ![Authorize the transaction](/media/images/docs/tutorials/parachains/auth-rococo-id.png)

1. Click **Network** and select **Explorer** to check the list of recent events for successful `registrar.Reserved`.
...
1. Set `relay-chain` to `rococo` and `para_id` to the identifier you've been assigned.
...
1. Save your changes and close the plain text chain specification file.
1. Generate a raw chain specification file from the modified chain specification file by running the following command:
...
1. Verify the parachain owner and the parachain identifier and upload the files that contain the WebAssembly validation function and the genesis state for the parachain, then click **Submit**.

1. Type your password to authenticate your identity, then click **Sign and Submit**.

1. Scroll down the list of Parathreads and verify that your parathread registration is **Onboarding**:
